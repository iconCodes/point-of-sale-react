...
export const useMenuDataStore = create<
  MenuDataStoreState & MenuDataStoreActions
>((set, get) => {
  ...
  return {
    ...
    addToCart: (item: MenuItem) => {
      ...
      if (currentDuplicateItems) {
        get().setQuantity(
          currentDuplicateItems.cartId,
          currentDuplicateItems.quantity + 1
        );
        return;
      }
        ...
        cartItems: state.cartItems.map(cartItem =>
          cartItem.cartId === cartItemId
            ? {
                ...
                quantity,
                totalPrice: Number(
                  (quantity * cartItem.pricePerUnit).toFixed(2)
                )
              }
            : cartItem
        )
